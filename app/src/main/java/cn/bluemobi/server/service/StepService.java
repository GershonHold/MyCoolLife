...
import cn.bluemobi.server.MainActivity;
import cn.bluemobi.server.R;
import cn.bluemobi.server.bean.StepsBean;
import cn.bluemobi.step.step.UpdateUiCallBack;
import cn.bluemobi.step.step.accelerometer.StepCount;
import cn.bluemobi.step.step.accelerometer.StepValuePassListener;
import cn.bluemobi.server.bean.LocalDbStepsBean;
import cn.bluemobi.step.step.config.Constant;
import cn.bluemobi.step.step.utils.DbUtils;

import static cn.bluemobi.server.service.StepsSendService.addStepsItems;
import static cn.bluemobi.server.service.StepsSendService.queryStepsItems;
import static cn.bluemobi.server.service.StepsSendService.updateStepsItems;
...
public class StepService extends Service implements SensorEventListener {
    ...
    String ID = "com.example.testmyweb.service";	//这里的id里面输入自己的项目的包的路径
    ...
    private void initTodayData() {
        ...
        if (list.size() == 0 || list.isEmpty()) {
            ...
            CURRENT_STEP = list.get(0).getSteps();
        } else {
            Log.v(TAG, "出错了！");
        }
        if (mStepCount != null) {
...
//                    save();
                    ...
                    saveToLocalDatabase();
                    if(!localmode){
                        saveToRemoteDatabase(CURRENT_DATE,CURRENT_STEP,usr_id);
                    }
...
//                    Logger.d("重置步数" + StepDcretor.CURRENT_STEP);
    ...
    public IBinder onBind(Intent intent) {
        ...
        System.out.println("onbid"+usr_id+name);
        initTodayData();
        return stepBinder;
    }
    ...
    private void saveToLocalDatabase() {
        ...
        List<LocalDbStepsBean> list = DbUtils.getQueryByWhere(LocalDbStepsBean.class, "date","name",new String[]{CURRENT_DATE},name);
        if (list.size() == 0 || list.isEmpty()) {
            LocalDbStepsBean localDbStepsBean = new LocalDbStepsBean();

            localDbStepsBean.setDate(CURRENT_DATE);
            localDbStepsBean.setSteps(tempStep);
            localDbStepsBean.setName(name);
            DbUtils.insert(localDbStepsBean);
        } else if (list.size() == 1) {
            LocalDbStepsBean localDbStepsBean = list.get(0);
            localDbStepsBean.setSteps(tempStep);
            localDbStepsBean.setName(name);
            DbUtils.update(localDbStepsBean);
        } else {
        }
    }

    public void saveToRemoteDatabase(String CURRENT_DATE,int steps,int usr_id){
        if(!queryStepsItems(CURRENT_DATE,0,usr_id)){
            StepsBean stepsBean = new StepsBean();
            stepsBean.setDate(CURRENT_DATE);
            stepsBean.setSteps(steps);
            stepsBean.setUsr_id(usr_id);
            addStepsItems(CURRENT_DATE,steps,usr_id);
            System.out.println(addStepsItems(CURRENT_DATE,steps,usr_id));
        } else{
//            updateStepsItems(CURRENT_DATE,tempStep,usr_id);
            System.out.println(updateStepsItems(CURRENT_DATE,steps,usr_id));
        }
    }
